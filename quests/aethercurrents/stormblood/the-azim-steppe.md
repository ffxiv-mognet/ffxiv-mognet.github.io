---
layout: aethercurrent-page
expansion: stormblood
title: The Azim Steppe | Stormblood Aether Currents 
permalink: /quests/aethercurrents/stormblood/the-azim-steppe
map: e3f3/00
nonsequential: true
links:
  next: /quests/aethercurrents/stormblood/the-lochs
  previous: /quests/aethercurrents/stormblood/yanxia
quests:
  - name: Glory to the Khagan
    level: 66
    rowId: 68052
    questId: StmBda417_02516
    genre:
      id: '6'
      name: Stormblood
    icon: '71000'
    issuer:
      location: The Azim Steppe
      coords: (31.2, 12.1)
      name: Hien
    steps:
      - location: The Azim Steppe
        coords: (31.4, 11.2)
        name: Speak with Temulun.
      - location: The Azim Steppe
        coords: (32.4, 11.8)
        name: Speak with the diligent Mol warrior.
      - location: The Azim Steppe
        coords: (32.4, 11.7)
        name: Strike a /victorypose at the mark of Mol Iloh.
      - location: The Azim Steppe
        coords: (31.4, 11.3)
        name: Speak with Cirina.
    unlocks:
      - id: '2818203'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Naadam
        link: /quests/msq/stormblood/part2
        level: 66
        rowId: 68051
        questId: StmBda416_02515
        genre: Stormblood
        icon: '71000'
    partQuestNo: 1
  - name: Words Are Very Unnecessary
    level: 65
    rowId: 68296
    questId: StmBdz509_02760
    genre:
      id: '131'
      name: Azim Steppe Sidequests
    icon: '71140'
    issuer:
      location: The Azim Steppe
      coords: (33.0, 28.3)
      name: Tamachag
    steps:
      - location: The Azim Steppe
        coords: (33.6, 28.0)
        name: Speak with Gascot.
      - location: The Azim Steppe
        coords: (33.2, 27.8)
        name: Speak with Gascot.
    unlocks:
      - id: '2818204'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: A Season for War
        link: /quests/msq/stormblood/part2
        level: 65
        rowId: 68037
        questId: StmBda402_02501
        genre: Stormblood
        icon: '71000'
    partQuestNo: 2
  - name: Sheep Snatcher
    level: 65
    rowId: 68307
    questId: StmBdz520_02771
    genre:
      id: '131'
      name: Azim Steppe Sidequests
    icon: '71140'
    issuer:
      location: The Azim Steppe
      coords: (28.3, 14.9)
      name: Sukegei
    steps:
      - location: The Azim Steppe
        coords: (19.6, 17.9)
        name: Use the blowgun to shoot the lost lamb from the designated location.
      - location: The Azim Steppe
        coords: (19.8, 18.5)
        name: Capture the lost lamb.
      - location: The Azim Steppe
        coords: (28.3, 14.9)
        name: Deliver the lost lamb to Sukegei.
    unlocks:
      - id: '2818205'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: The Heart of Nations
        link: /quests/msq/stormblood/part2
        level: 65
        rowId: 68041
        questId: StmBda406_02505
        genre: Stormblood
        icon: '71000'
    partQuestNo: 3
  - name: Forty Years and Counting
    level: 66
    rowId: 68318
    questId: StmBdz604_02782
    genre:
      id: '131'
      name: Azim Steppe Sidequests
    icon: '71140'
    issuer:
      location: The Azim Steppe
      coords: (23.0, 23.4)
      name: Esugen
    steps:
      - location: The Azim Steppe
        coords: (22.1, 20.6)
        name: Speak with Sorocan.
      - location: The Azim Steppe
        coords: (22.0, 20.6)
        name: Obtain pails of fresh sheep's milk.
      - location: The Azim Steppe
        coords: (23.0, 23.4)
        name: Deliver the fresh sheep's milk to Esugen.
    unlocks:
      - id: '2818206'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: For Love of the Moon
        link: /quests/msq/stormblood/part2
        level: 66
        rowId: 68046
        questId: StmBda411_02510
        genre: Stormblood
        icon: '71000'
    partQuestNo: 4
  - name: Mauci of the Seven Worries
    level: 66
    rowId: 68327
    questId: StmBdz613_02791
    genre:
      id: '131'
      name: Azim Steppe Sidequests
    icon: '71140'
    issuer:
      location: The Azim Steppe
      coords: (12.8, 34.5)
      name: Mauci
    steps:
      - location: The Azim Steppe
        coords: (12.3, 33.4)
        name: Gather information in Dotharl Khaa.
      - location: The Azim Steppe
        coords: (12.8, 34.5)
        name: Report to Mauci.
    unlocks:
      - id: '2818207'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: The Undying Ones
        link: /quests/msq/stormblood/part2
        level: 66
        rowId: 68048
        questId: StmBda413_02512
        genre: Stormblood
        icon: '71000'
    partQuestNo: 5


---