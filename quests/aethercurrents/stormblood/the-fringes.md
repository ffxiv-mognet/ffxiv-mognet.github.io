---
layout: aethercurrent-page
expansion: stormblood
title: The Fringes | Stormblood Aether Currents 
permalink: /quests/aethercurrents/stormblood/the-fringes
map: g3f1/00
nonsequential: true
links:
  next: /quests/aethercurrents/stormblood/the-peaks
quests:
  - name: The Silence of the Gods
    level: 67
    rowId: 68069
    questId: StmBda607_02533
    genre:
      id: '6'
      name: Stormblood
    icon: '71000'
    issuer:
      location: The Fringes
      coords: (31.7, 16.7)
      name: Vajra
    steps:
      - location: The Fringes
        coords: (28.0, 19.4)
        name: Speak with Sarisha.
      - location: The Fringes
        coords: (30.0, 25.9)
        name: Speak with M'naago.
      - location: The Fringes
        coords: (30.0, 25.9)
        name: Speak with M'naago.
      - location: The Fringes
        coords: (30.0, 25.9)
        name: Speak with Alphinaud.
    unlocks:
      - id: '2818128'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: The Lady of Bliss
        level: 67
        rowId: 68068
        questId: StmBda606_02532
        genre: Stormblood
        icon: '71000'
        link: /quests/msq/stormblood/part2
    partQuestNo: 1
  - name: Magiteknical Failure
    level: 60
    rowId: 68175
    questId: StmBdz005_02639
    genre:
      id: '125'
      name: Fringes Sidequests
    icon: '71140'
    issuer:
      location: The Fringes
      coords: (8.4, 11.2)
      name: Loezkirz
    steps:
      - location: The Fringes
        coords: (9.0, 11.8)
        name: Speak with Lieutenant G'nibnpha.
      - location: The Fringes
        coords: (12.4, 13.4)
        name: Pilot the magitek armor to East End and destroy the Garlean drones.
      - location: The Fringes
        coords: (16.6, 7.7)
        name: Pilot the magitek armor to Velodyna River and destroy the Garlean drones.
      - location: The Fringes
        coords: (8.4, 11.2)
        name: Report to Captain Loezkirz.
    unlocks:
      - id: '2818129'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Signed, Sealed, to Be Delivered
        level: 60
        rowId: 67988
        questId: StmBda112_02452
        genre: Stormblood
        icon: '71000'
        link: /quests/msq/stormblood/part1
    partQuestNo: 2
  - name: The Hidden Truth
    level: 61
    rowId: 68197
    questId: StmBdz111_02661
    genre:
      id: '125'
      name: Fringes Sidequests
    icon: '71140'
    issuer:
      location: The Fringes
      coords: (9.2, 10.8)
      name: Serpent Marshal Brookstone
    steps:
      - location: The Fringes
        coords: (8.6, 10.7)
        name: Speak with Lieutenant Molkoh.
      - location: The Fringes
        coords: (8.8, 12.1)
        name: Speak with the Ananta battlemaid.
      - location: The Fringes
        coords: (12.9, 19.6)
        name: Speak with Lieutenant Molkoh.
      - location: The Fringes
        coords: (12.8, 19.1)
        name: Interrogate the startled soldier.
      - location: The Fringes
        coords: (12.9, 19.0)
        name: Aid Lieutenant Molkoh.
    unlocks:
      - id: '2818130'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Divide and Conquer
        level: 61
        rowId: 68002
        questId: StmBda136_02466
        genre: Stormblood
        icon: '71000'
        link: /quests/msq/stormblood/part1
    partQuestNo: 3
  - name: Unexpected Guests
    level: 67
    rowId: 68352
    questId: StmBdz713_02816
    genre:
      id: '125'
      name: Fringes Sidequests
    icon: '71140'
    issuer:
      location: The Fringes
      coords: (27.0, 21.0)
      name: Prapti
    steps:
      - location: The Fringes
        coords: (32.6, 24.2)
        name: Head to the Vira hunting grounds.
      - location: The Fringes
        coords: (32.8, 24.2)
        name: Speak with the Vira bowmaid.
      - location: The Fringes
        coords: (34.1, 28.0)
        name: Speak with the strapping leader.
      - location: The Fringes
        coords: (27.0, 21.0)
        name: Report to Prapti.
    unlocks:
      - id: '2818131'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: The Lure of the Dream
        level: 67
        rowId: 68067
        questId: StmBda605_02531
        genre: Stormblood
        icon: '71000'
        link: /quests/msq/stormblood/part2
    partQuestNo: 4
  - name: Eyes Bigger than Her Stomach
    level: 67
    rowId: 68357
    questId: StmBdz718_02821
    genre:
      id: '125'
      name: Fringes Sidequests
    icon: '71140'
    issuer:
      location: The Fringes
      coords: (30.1, 26.1)
      name: M'zimzizi
    steps:
      - location: The Fringes
        coords: (29.3, 23.6)
        name: Use burlap sacks on weakened teleocerases.
      - location: The Fringes
        coords: (30.1, 26.1)
        name: Deliver the dinosaur-filled sacks to M'zimzizi.
    unlocks:
      - id: '2818132'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Rising Fortunes, Rising Spirits
        level: 67
        rowId: 68066
        questId: StmBda604_02530
        genre: Stormblood
        icon: '71000'
        link: /quests/msq/stormblood/part2
    partQuestNo: 5


---