---
layout: aethercurrent-page
expansion: heavensward
title: The Churning Mists| Heavensward Aether Currents 
permalink: /quests/aethercurrents/heavensward/churning-mists
map: d2f3/00
nonsequential: true
links:
  previous: /quests/aethercurrents/heavensward/dravanian-hinterlands
  next: /quests/aethercurrents/heavensward/sea-of-clouds
quests:
  - name: Into the Aery
    level: 55
    rowId: 67170
    questId: HeaVna333_01634
    genre:
      id: '3'
      name: Heavensward
    icon: '71000'
    issuer:
      location: Foundation
      coords: (13.4, 11.0)
      name: Estinien
    steps:
      - location: Foundation
        coords: (7.5, 10.2)
        name: Speak with Cid.
      - location: The Aery
        coords: (11.1, 7.0)
        name: Enter the Aery.
      - location: The Churning Mists
        coords: (33.6, 15.4)
        name: Enter the Aery.
      - location: The Churning Mists
        coords: (33.5, 15.4)
        name: Speak with Estinien.
    unlocks:
      - id: '2818093'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Ready to Fly
        level: 55
        rowId: 67169
        questId: HeaVna332_01633
        genre: Heavensward
        icon: '71000'
    partQuestNo: 1
  - name: The Bathing Bully
    level: 54
    rowId: 67355
    questId: HeaVnz401_01819
    genre:
      id: '121'
      name: Churning Mists Sidequests
    icon: '71140'
    issuer:
      location: The Churning Mists
      coords: (29.8, 36.1)
      name: Mogret
    steps:
      - location: The Churning Mists
        coords: (36.6, 32.5)
        name: Slay hropkens.
      - location: The Churning Mists
        coords: (29.8, 36.1)
        name: Report to Mogret.
    unlocks:
      - id: '2818094'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Moghan's Trial
        level: 54
        rowId: 67156
        questId: HeaVna319_01620
        genre: Heavensward
        icon: '71000'
    partQuestNo: 2
  - name: Road Kill
    level: 54
    rowId: 67359
    questId: HeaVnz405_01823
    genre:
      id: '121'
      name: Churning Mists Sidequests
    icon: '71140'
    issuer:
      location: The Churning Mists
      coords: (27.5, 33.8)
      name: Mogsy
    steps:
      - location: The Churning Mists
        coords: (31.6, 24.6)
        name: Slay archaeosaurs.
      - location: The Churning Mists
        coords: (21.6, 25.9)
        name: Report to Mogmont.
    unlocks:
      - id: '2818095'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Leaving Moghome
        level: 54
        rowId: 67160
        questId: HeaVna323_01624
        genre: Heavensward
        icon: '71000'
    partQuestNo: 3
  - name: The Unceasing Gardener
    level: 54
    rowId: 67365
    questId: HeaVnz411_01829
    genre:
      id: '121'
      name: Churning Mists Sidequests
    icon: '71020'
    issuer:
      location: The Churning Mists
      coords: (12.2, 32.3)
      name: manic moogle
    steps:
      - location: The Churning Mists
        coords: (12.2, 32.3)
        name: Get the high-quality blackloam from the clay pot.
      - location: The Churning Mists
        coords: (29.8, 36.1)
        name: Deliver the blackloam to Mogomo.
    unlocks:
      - id: '2818096'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: The Wyrm's Lair
        level: 54
        rowId: 67164
        questId: HeaVna327_01628
        genre: Heavensward
        icon: '71000'
    partQuestNo: 4
  - name: Waiting to Inhale
    level: 54
    rowId: 67371
    questId: HeaVnz417_01835
    genre:
      id: '121'
      name: Churning Mists Sidequests
    icon: '71140'
    issuer:
      location: The Churning Mists
      coords: (32.9, 15.4)
      name: moon-eyed moogle
    steps:
      - location: The Churning Mists
        coords: (33.9, 16.1)
        name: Slay tulihands.
      - location: The Churning Mists
        coords: (32.9, 15.4)
        name: Report to the moogle on lookout duty.
    unlocks:
      - id: '2818097'
        name: Aether Current
        type: aethercurrent
    requires:
      - name: Heart of Ice
        level: 54
        rowId: 67163
        questId: HeaVna326_01627
        genre: Heavensward
        icon: '71000'
    partQuestNo: 5




---